...
where athlete_id = athletes.id and assessment_data.date > '2022-07-01'"
...
df_reshaped <- df_reshaped%>%
  filter(!is.na(`throwing-side`))

write.csv(df_reshaped, "assessment_data_post_July_2022.csv", row.names = FALSE)
